...
> **보관용 문서 (2026-06):** 이 sweep을 돌리던 스크립트(`write_antmaze_res_subgoal_grid_yaml.py`, `sweep_antmaze_res_subgoal_grid_600ep.sh`, `nohup_sweep_res_subgoal_grid_all.sh` 등)는 저장소에서 제거되었습니다. 동일 축의 고정 YAML은 `config/grid_targetmode/`에 남아 있고, 현재 유지되는 신규 sweep 경로는 Flow+TRL 계열 `scripts/run_flow_trl_*`입니다.
