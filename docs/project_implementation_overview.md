...
Success 판정은 OGBench env의 `info["success"]`를 기준으로 합니다. `subgoal_filter`가 켜져 있으면 critic value를 이용해 나쁜 subgoal을 goal representation channel 기준으로 보정할 수 있습니다.
...
- `dataset_dir`, `subgoal_filter`, `subgoal_override_goal`, `eval_video_*`, `goal_representation=phi`는 README에 보강하면 좋습니다.
