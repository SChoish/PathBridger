...
PathBridger는 OGBench의 long-horizon offline control 문제를 풀기 위해, 최종 goal로 바로 action을 예측하지 않고 다음 세 단계를 함께 학습합니다.
...
Subgoal이 정해지면 dynamics는 현재 상태에서 subgoal까지의 상태 trajectory를 만듭니다. 현재 메인 planner는 `forward_bridge_residual`입니다.

- closed-form forward bridge mean을 base path로 사용합니다.
- `PathResidualNet`이 endpoint-preserving residual로 dataset trajectory에 맞는 내부 경로 보정항을 예측합니다.
...
- `PathResidualNet`: closed-form forward bridge mean 위에 더할 endpoint-preserving residual 예측
...
현재 기본값은 `forward_bridge_residual + deterministic subgoal + prefix_progress + hard bridge`입니다.
