...
Checkpoint 기반 eval 결과를 JSON으로 저장하고, 이미 저장된 결과는 `--skip_if_saved`로 건너뜁니다. 최근에는 `eval_max_chunks`가 결과 파일명과 record에 포함되도록 정리되어, 같은 checkpoint를 200 chunks와 800 chunks로 구분할 수 있습니다.
...
- eval budget: `eval_max_chunks=200`, action chunk horizon 5, 즉 1000 env steps
...
`scripts/run_amg_m800_eval.sh`는 antmaze-giant checkpoint를 `eval_max_chunks=800`으로 재평가합니다. 800 chunks는 action chunk horizon 5 기준 4000 env steps입니다.

관련 chain script:

- `scripts/run_amg_m800_then_p456.sh`: giant m800 eval 후 puzzle 4x5/4x6 sweep 재개
- temp: `1.0`, `0.5`
- eval N: 현재 `{2, 8, 16, 32}`
- 결과 JSON filename에 `_m800_`이 들어갑니다.
...
- `eval_checkpoint.py`가 `flags.json`을 읽어 checkpoint hyperparameter를 복원하므로, 훈련 이후 N/temp/max_chunks eval sweep을 붙이기 쉽습니다.
...
Puzzle 4x5/4x6와 giant m800 같은 sweep은 nohup chain으로 오래 돕니다. 중간에 중지/재시작할 때는 run_group, gamma, eval JSON completeness를 기준으로 skip/retrain 여부가 결정됩니다.
