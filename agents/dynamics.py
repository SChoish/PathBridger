...
class SubgoalFlowNet(nn.Module):
    ...
    def __call__(self, observations, high_actor_goals, x_u, u):
        ...
        if u.ndim == 2 and u.shape[-1] == 1:
            ...
            u_scalar = u
        if self.use_time_embedding:
            t_inp = SinusoidalEmbedding(self.time_embed_dim)(u_scalar)
        else:
    ...
    def create(cls, seed, ex_observations, config, ex_actions=None):
        ...
        if sub_mode == 'deterministic':
            ...
            subgoal_def = SubgoalFlowNet(
                hidden_dims=tuple(config['subgoal_hidden_dims']),
                state_dim=state_dim,
                time_embed_dim=int(config.get('subgoal_flow_time_embed_dim', config['time_embed_dim'])),
                use_time_embedding=bool(config.get('subgoal_flow_use_time_embedding', True)),
                layer_norm=config['layer_norm'],
                goal_representation=str(
                    config.get('subgoal_goal_representation', config.get('goal_representation', 'full')),
                ),
...
class DynamicsAgent(_DynamicsAgentCore):
    ...
    def _subgoal_bonus_type_id(self) -> jnp.ndarray:
        ...
        ids = {
            'none': 0.0,
            'single_value': 1.0,
            'transitive_product': 2.0,
            'transitive_ratio': 3.0,
        }
    ...
    def _subgoal_value_terms(
        ...
        if self._is_trl_mode() and critic_value_params is not None:
            ...
            elif bonus_type == 'transitive_product':
                raw_bonus = transitive_product
            elif bonus_type == 'transitive_ratio':
                raw_bonus = transitive_ratio
            else:
                raise ValueError(
                    "subgoal_value_bonus_type must be one of 'none', 'single_value', "
                    f"'transitive_product', or 'transitive_ratio', got {bonus_type!r}."
                )
            subgoal_value_bonus = jnp.where(alpha > 0.0, alpha * raw_bonus, jnp.zeros_like(pred_value))
        else:
            v_s_sg = jnp.zeros_like(pred_value)
            v_sg_g = pred_value
            if bonus_type == 'none':
                raw_bonus = jnp.zeros_like(pred_value)
            ...
            subgoal_value_bonus = jnp.where(
                alpha > 0.0,
                alpha * raw_bonus,
                jnp.zeros_like(pred_value),
            )
        return (
            pred_value,
            obs_value,
    ...
    def _compute_subgoal_loss(self, batch, grad_params, rng_fr, critic_value_params):
        ...
        if _subgoal_spi_enabled(self.config):
            ...
            velocity_reg = float(self.config.get('subgoal_flow_velocity_reg', 0.0))
            velocity_norm_sq = jnp.sum(v_pred ** 2, axis=-1)
            loss_sub = loss_fm - jnp.mean(subgoal_value_bonus) + velocity_reg * jnp.mean(velocity_norm_sq)
            ...
            subgoal_extra_info = {
                ...
                'phase1/subgoal_v_s_g_mean': jnp.mean(current_value),
                'phase1/subgoal_transitive_product_mean': jnp.mean(subgoal_transitive_product),
                'phase1/subgoal_transitive_ratio_mean': jnp.mean(subgoal_transitive_ratio),
            }
        elif sub_mode == 'diag_gaussian':
            stochastic_loss = _subgoal_stochastic_loss(self.config)
            pred_mu, pred_log_std = self.network.select('subgoal_net')(
