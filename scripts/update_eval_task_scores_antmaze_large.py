...
``runs/…`` copy from all ``runs/*_seed0_antmaze-large-navigate-v0`` runs.

Preserves the appendix + notes suffix (from ``### 표 해석 메모`` onward) in the existing docs file.
YAML에 없는 ``dynamics`` / ``actor`` / top-level 키는 ``get_dynamics_config`` / ``get_actor_config`` / ``main`` 기본값으로 채움.
"""
...
RUN_GLOB = "*_seed0_antmaze-large-navigate-v0"
...
_HEATMAP_BLURB = (
    ...
    "생성: `python scripts/plot_dynamics_tau_alpha_sweep_heatmaps.py`\n\n"
    "(legacy GOUB sweep 자료는 ``config/legacy/`` / ``scripts/legacy/`` 참조.)\n\n"
)
...
def merge_top_level(cfg: dict) -> dict[str, Any]:
    ...
    for k, v in cfg.items():
        if k in ("dynamics", "goub", "critic_agent", "actor") or v is None:
            continue
        out[k] = v
    return out


def format_run_diff_markdown(cfg: dict) -> str:
    g = merge_dynamics(cfg)
    a = merge_actor(cfg)
...
def build_tables_markdown(rows: list[dict[str, Any]]) -> str:
    ...
    for r in rows:
        lines.append(f"| **{r['short']}** | {format_run_diff_markdown(r['cfg'])} |\n")
...
def _docs_header(*, updated_line: str) -> str:
    return (
        "# AntMaze large — 런별 설정 요약 및 EVAL mean (ep100–600)\n\n"
...
def main() -> None:
    ...
    updated = (
        ...
        "`python scripts/update_eval_task_scores_antmaze_large.py` 로 재생성."
    )
