#!/usr/bin/env bash
# 1) antmaze-giant env-max eval (temp 1.0 / 0.5)
...
MASTER_LOG="${LOG_DIR}/amg_envmax_then_p456_master.log"

echo "[$(date -Is)] amg_envmax_then_p456 start" | tee -a "${MASTER_LOG}"
...
echo "[$(date -Is)] amg_envmax_then_p456 complete" | tee -a "${MASTER_LOG}"
