#!/usr/bin/env bash
# Train + final eval: humanoidmaze medium/large × gap {5,10} × distance_weight {0.0,0.1}.
# Paper: gamma=0.999; policy (0,0,1,0); TRL value (0,0,1,0).
set -euo pipefail
...
RUNS_ROOT="${ROOT}/runs"
CONFIG_DIR="${ROOT}/config/sweep_flow_trl_humanoidmaze_dw"
FINAL_EPOCH="${FINAL_EPOCH:-600}"
LOG_TAG="flow_trl_humanoidmaze_dw"
FINAL_EVAL_NS=(2 8 16 32)
# humanoidmaze TimeLimit=2000 env steps; h_a=5 → 400 outer replans.
HUMANOID_ENV_STEPS="${HUMANOID_ENV_STEPS:-2000}"
EVAL_RESULT_SUFFIX="${EVAL_RESULT_SUFFIX:-cap2000}"

mkdir -p "${LOG_DIR}"
MASTER_LOG="${LOG_DIR}/${LOG_TAG}_master.log"

"${PYTHON_BIN}" "${ROOT}/scripts/generate_flow_trl_humanoidmaze_dw_sweep_configs.py"
...
eval_done() {
  local run_dir="$1"
  local eval_n="$2"
  [[ -f "${run_dir}/eval_results/epoch${FINAL_EPOCH}_n${eval_n}_${EVAL_RESULT_SUFFIX}.json" ]]
}

eval_max_chunks_for_cfg() {
  local cfg="$1"
...
cfg = yaml.safe_load(open(os.environ['CONFIG_PATH'], encoding='utf-8'))
print(int(cfg.get('eval_max_chunks', 400)))
PY
}
...
run_final_evals() {
  ...
  local run_dir="$2"
  local cfg="$3"
  local eval_max_chunks
  eval_max_chunks="$(eval_max_chunks_for_cfg "${cfg}")"
  local eval_n
  for eval_n in "${FINAL_EVAL_NS[@]}"; do
    if eval_done "${run_dir}" "${eval_n}"; then
      echo "[$(date -Is)] SKIP_EVAL_SAVED ${base} eval_n=${eval_n} suffix=${EVAL_RESULT_SUFFIX}" | tee -a "${MASTER_LOG}"
      continue
    fi
    eval_log="${LOG_DIR}/${LOG_TAG}_${base}.eval_n${eval_n}.${EVAL_RESULT_SUFFIX}.log"
    echo "[$(date -Is)] START_EVAL ${base} eval_n=${eval_n} max_chunks=${eval_max_chunks} suffix=${EVAL_RESULT_SUFFIX} run_dir=${run_dir}" | tee -a "${MASTER_LOG}"
    CUDA_VISIBLE_DEVICES="${GPU_ID}" bash "${WITH_CUDA}" "${PYTHON_BIN}" -u eval_checkpoint.py \
      ...
      --eval_result_suffix "${EVAL_RESULT_SUFFIX}" \
      --subgoal_eval_num_samples "${eval_n}" \
      --mujoco_gl "${MUJOCO_GL}" \
      --skip_if_saved \
      2>&1 | tee "${eval_log}"
    echo "[$(date -Is)] DONE_EVAL ${base} eval_n=${eval_n} suffix=${EVAL_RESULT_SUFFIX}" | tee -a "${MASTER_LOG}"
  done
}
