...
# AntMaze linear dynamics sweep.
# - large: 300 epochs, skipping alpha=0.3 / tau=10 in YAML because
#   runs/20260425_224314_joint_dqc_seed0_antmaze-large-navigate-v0 covers it
#   (same env / tau / alpha; heatmap script maps that run into the grid cell).
# - giant: 500 epochs for the full alpha in {0.1, 0.3, 0.5} and
#   spi_tau in {5, 10} grid; critic (and logged actor) discount **0.995**.
#   Re-run giant only: `scripts/launch_dynamics_tau_alpha_sweep_giant_only.sh`.
