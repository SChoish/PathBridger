#!/usr/bin/env bash
# Actor SPI tau sweep for all 1M best-IDM checkpoints (checkpoints/1m_env_best/).
#
# Usage:
#   bash scripts/run_actor_spi_1m_sweep.sh [seed] [tau1 tau2 ...]
#
# Per-env N/T come from each bundle's best_eval_meta.yaml (manifest selection).
...
LOG_DIR="${ROOT}/nohup_logs/actor_spi/1m"
MASTER="${LOG_DIR}/master_seed${SEED}.log"
mkdir -p "${LOG_DIR}"

# env -> bundle dir (must match checkpoints/1m_env_best/manifest.yaml)
declare -A ENVS=(
...
echo "[$(date -Is)] actor_spi 1m sweep start seed=${SEED} taus=${TAUS[*]} epoch=${PRETRAINED_EPOCH}" | tee -a "${MASTER}"
...
echo "[$(date -Is)] actor_spi 1m sweep complete seed=${SEED}" | tee -a "${MASTER}"
