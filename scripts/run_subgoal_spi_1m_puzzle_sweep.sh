#!/usr/bin/env bash
# Subgoal SPI tau sweep for 1M puzzle checkpoints only.
#
# Usage:
#   bash scripts/run_subgoal_spi_1m_puzzle_sweep.sh [seed] [tau1 tau2 ...]
...
LOG_DIR="${ROOT}/nohup_logs/subgoal_spi/1m_puzzle"
...
echo "[$(date -Is)] subgoal_spi 1m puzzle sweep start seed=${SEED} taus=${TAUS[*]} epoch=${PRETRAINED_EPOCH}" | tee -a "${MASTER}"
...
echo "[$(date -Is)] subgoal_spi 1m puzzle sweep complete seed=${SEED}" | tee -a "${MASTER}"
