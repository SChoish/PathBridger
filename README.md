...
| 키 | 기본 | 설명 |
|----|------|------|
| `planner_type` | `exact_residual_chain` | exact bridge posterior mean + learned residual reverse chain. 대안: `forward_bridge` (closed-form forward bridge mean, 학습 path 파라미터 없음), `forward_bridge_residual` (forward bridge mean + endpoint-preserving 학습 residual `PathResidualNet`) |
...
환경마다 baseline config 1개씩 운영합니다. 알고리즘 default(`prefix_progress` + `subgoal_value_alpha=0.5` + `spi_tau=10.0` + `final_eval_episodes_per_task=50`)는 yaml에서 생략하고, **환경별 critic 차이만 yaml에 명시**합니다.
